...
Department.drop_table()  # deletes table

Department.create_table()  # creates table with no data
# CURSOR.execute("PRAGMA table_info(departments)").fetchall()   # PRAGMA statement: shows info about the created table

#======== below is wrapped in create function ======
...
# ==== Querying the database to extract a list of items
departments = CURSOR.execute('SELECT * FROM departments')
[row for row in departments]							# This will return each row with assoc. info. in a tuple
# => [(1, 'Payroll', 'Building A, 5th Floor'), (2, 'Human Resources', 'Building C, East Wing')]

# exit()    # exits out of pdb
